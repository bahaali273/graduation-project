   ...
   if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($_POST['submit'])){
     ...
     $con = new mysqli("localhost","root","","traino");
         $st=$con->prepare("select 	password from academicadvisor where academicid ='$id'");
         $st->execute();
         $rs=$st->get_result();
         $row=$rs->fetch_assoc();
         echo $row["password"];
         if ($password == $row["password"]) {
            ...
            if($newpassword === $confirmnewpassword){
                ...
                $stm=$con->prepare("update academicadvisor set password = '$newpassword' where academicid ='$id'");
                $stm->execute();
